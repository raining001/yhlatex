...
class MultiAttention(nn.Module):
    ...
    * Bahdanau Attention (mlp):
    ...
    def __init__(self, outdim, dim, coverage=False, attn_type="dot",
                 attn_func="softmax"):
        ...
        if self.attn_type == "general":
            self.linear_in = nn.Linear(dim, outdim, bias=False)
        elif self.attn_type == "mlp":
            self.linear_context = nn.Linear(dim, outdim, bias=False)
            self.linear_query = nn.Linear(dim, outdim, bias=True)
            self.v = nn.Linear(dim, 1, bias=False)
        # mlp wants it with bias
        out_bias = self.attn_type == "mlp"
        self.linear_out = nn.Linear(dim * 2, outdim, bias=out_bias)

        if coverage:
            self.linear_cover = nn.Linear(1, outdim, bias=False)

    def score(self, h_t, h_s):          # (h_t(bz,1, 512), 对应公式中h, h_s，对用公式中的V (bz, wxh, 512))
        """
        Args:
        ...
        if self.attn_type in ["general", "dot"]:
            ...
            if self.attn_type == "general":
                ...
                h_t = h_t_.view(tgt_batch, tgt_len, -1)  #(5, 1, 512)
            h_s_ = h_s.transpose(1, 2)                          #()
            # (batch, t_len, d) x (batch, d, s_len) --> (batch, t_len, s_len)
            # print('h_t', h_t.size())
            # print('h_s_', h_s_.size())
            ...
            return self.v(wquh.view(-1, dim)).view(tgt_batch, tgt_len, src_len)    # at;h;w = βT tanh(W1ht + W2Vhw)
    ...
    def forward(self, source, memory_bank, memory_lengths=None, coverage=None):
        ...
        align = self.score(source, memory_bank)     # 对应公式  计算attention权重公式     et = a(ht; fV~ h;wg) 已经是
