...
def model_opts(parser):
    ...
    group.add('--encoder_type', '-encoder_type', type=str, default='rnn',
              choices=['rnn', 'brnn', 'mean', 'transformer', 'cnn', 'imgDense'],
              help="Type of encoder layer to use. Non-RNN layers "
                   "are experimental. Options are "
                   "[rnn|brnn|mean|transformer|cnn|style].")
    ...
    group.add('--log-path', dest="log_path",
                        type=str, default='train_log.txt',
    ...
    group.add("--growthrate", "-growthrate", type = int, default = 24)
...
def preprocess_opts(parser):
    ...
    group.add('--log_file', '-log_file', type=str, default="log_train.txt",
...
def translate_opts(parser):
    ...
    group = parser.add_argument_group('Data')
    group.add('--data_type', '-data_type', default="img",
              help="Type of the source input. Options: [text|img].")

    group.add('--src', '-src', required=True, default="data/im2text/src-test.txt",
              help="Source sequence to decode (one line per "
                   "sequence)")
    group.add('--src_dir', '-src_dir', default='data/im2text/images_test',
              help='Source directory for image or audio files')
    group.add('--tgt', '-tgt',
    ...
    group.add('--shard_size', '-shard_size', type=int, default=10000,
              help="Divide src and tgt (if applicable) into "
                   ...
                   "shard_size>0 means segment dataset into multiple shards, "
                   "each shard has shard_size samples")
    group.add('--output', '-output', default='results/pred.txt',
              help="Path to output the predictions (each line will "
                   "be the decoded sequence")
    group.add('--score', '-score', default='results/pred_score.txt',
              help="Path to translate score")
