...
def build_translator(opt, report_score=True, logger=None, out_file=None, score_file=None):
    if out_file is None:
        out_file = codecs.open(opt.output, 'w+', 'utf-8')
    if score_file is None:
        score_file = codecs.open(opt.score, 'w+', 'utf-8')
...
class Translator(object):
    ...
    def savetxt(self, ignoretxt, out_dir):
        fw = open(out_dir, 'w')  # 将要输出保存的文件地址
        for line in ignoretxt:  # 读取的文件
            fw.write(str(line))  # 将字符串写入文件中
        fw.close()
    ...
    def translate(
        ...
        # data:src 指 图片 _data [('src', [b'0.png\n', b'1.png\n', b'2.png\n', b'3.png\n', b'4.png\n'])]
        ...
        for batch in data_iter:
            ...
            for trans in translations:
                ...
                self.out_file.write('\n'.join(n_best_preds) + '\n')
                self.out_file.flush()
                score = trans.pred_scores[0].cpu().numpy().tolist()
                ...
                if attn_debug:
                    ...
                    if attn_view:
                        fw = open('error_lab/attn.txt', 'w')  # 将要输出保存的文件地址
                        for line in attns:  # 读取的文件
                            line = [str(i) for i in line]
                            line = "\t".join(line)
                            # print(line)
                            fw.write(line + '\n')  # 将字符串写入文件中
                        fw.close()
                        # vis_img_with_attention()
                    ...
                    if self.logger:
                        ...
                        # print(type(output))
                        # print(output)
                        self.savetxt(output, 'error_lab/debug.txt')
                        # os.write(1, output.encode('utf-8'))
                ...
                if align_debug:
                    ...
                    if self.logger:
                        ...
                        # print(output)
                        # self.savetxt(output.encode('utf-8'), 'error_lab/align.txt')
                        os.write(1, output.encode('utf-8'))
    ...
    def _decode_and_generate(
        ...
        # print('dec_out', dec_out.size())            # 512
    ...
    def _translate_batch_with_strategy(
        ...
        # (3) Begin decoding step by step:  decode_strategy.max_length 150
        probss = []
    ...
    def debug_probs(self, log_probs, step, topk=5):
        tgt_field = dict(self.fields)["tgt"].base_field
        ...
        for i in range(1):
            for j in range(topk):
                ...
                print(vocab.itos[idx], np.max(probs[i]))
                probs[i][idx] = -99
            print()
