...
viz = Visdom(env='im2lx', port=8099)
...
class ReportMgr(ReportMgrBase):
    ...
    def _report_training(self, step, num_steps, learning_rate,
        ...
        xx = torch.tensor([step])
        loss = torch.tensor([report_stats.loss])
        ppl = torch.tensor([report_stats.ppl()])
        accuracy = torch.tensor([report_stats.accuracy()])
        viz.line(loss, xx, win='loss', update='append', opts=dict(title="loss"))
        viz.line(ppl, xx, win='ppl', update='append', opts=dict(title="ppl"))
        viz.line(accuracy, xx, win='accuracy', update='append', opts=dict(title="accuracy"))
